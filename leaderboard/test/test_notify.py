...
from notify import merge_scores, get_events, get_event_history, condense_events


def mocked_get_score_events(previous_score, score):
...
class TestNotify(unittest.TestCase):
    ...
    def test_get_event_history(self, get_score_events):
        acc_scores = {
            "user1": [0, 50, 100],
            "user2": [0, 20, 70],
        }
        expected_event_history = {"user1": [None, "finish"], "user2": [None, None]}
    ...
    def test_condense_events(self, get_score_events):
        ...
        self.assertEqual(
            condense_events(acc_events1, events1, clock), expected_condensed_events1
        )
        self.assertEqual(
            condense_events(acc_events2, events2, clock), expected_condensed_events2
        )
